...
import { DynamicNewsArticle } from "@/components/FeaturedNews";
...
export function NewsClient({ initialNews }: { initialNews?: DynamicNewsArticle[] }) {
  ...
  const pageTitle = t("Coffee News & Information", "أخبار ومعلومات القهوة", "هەواڵ و زانیارییەکانی قاوە");
    ...
    <div className="w-full bg-[#F8F7F5] dark:bg-bg-primary text-text-primary transition-colors duration-300 min-h-screen pt-28 lg:pt-36 pb-20">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-12 lg:mb-16 max-w-3xl">
          <span className="text-xs lg:text-sm font-semibold text-gold uppercase tracking-widest block mb-3">
            Diar Journal
          </span>
          <h1 className="font-serif text-4xl sm:text-5xl lg:text-6xl text-[#1E1611] dark:text-text-primary font-bold tracking-tight mb-4 leading-tight">
            {pageTitle}
          </h1>
          <p className="font-sans text-text-secondary text-base sm:text-lg max-w-2xl leading-relaxed">
            {pageSubtitle}
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
            ...
            {featuredArticle && (
              <Link 
                ...
                className="col-span-1 md:col-span-2 lg:col-span-3 bg-white dark:bg-[#141414] rounded-[16px] overflow-hidden border border-[#E5E0D8] dark:border-white/10 shadow-sm flex flex-col transition-all duration-300 hover:shadow-md hover:border-gold/40 cursor-pointer group/card block"
              >
                ...
                <div className="p-6 sm:p-8 flex flex-col flex-grow justify-between">
                  <div>
                    <h2 className="font-serif text-2xl sm:text-3xl text-[#251B12] dark:text-white font-bold tracking-tight mb-3 hover:text-gold transition-colors duration-200">
            ...
            {regularArticles.map((article) => (
              <Link 
                ...
                className="bg-white dark:bg-[#141414] rounded-[16px] overflow-hidden border border-[#E5E0D8] dark:border-white/10 shadow-sm flex flex-col transition-all duration-300 hover:shadow-md hover:border-gold/40 cursor-pointer group/card block"
              >
            ...
            {news.length === 0 && (
              <div className="col-span-1 md:col-span-2 lg:col-span-3 text-center py-20 text-text-secondary">
